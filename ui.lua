...
function Library:SelectPage(pageName)
    for _, page in pairs(self.Pages) do
        if page.Name == pageName then
            if self.ActivePage then
                ...
                Utility:Tween(self.ActivePage.Button, {BackgroundColor3 = self.Theme.DarkContrast}, 0.3)
                Utility:Tween(self.ActivePage.Title, {TextColor3 = self.Theme.SubTextColor}, 0.3)
                Utility:Tween(self.ActivePage.Icon, {ImageColor3 = self.Theme.SubTextColor}, 0.3)
            ...
            Utility:Tween(page.Button, {BackgroundColor3 = self.Theme.Accent}, 0.3)
            Utility:Tween(page.Title, {TextColor3 = self.Theme.TextColor}, 0.3)
            Utility:Tween(page.Icon, {ImageColor3 = self.Theme.TextColor}, 0.3)
    ...
    -- Apply smooth scrolling to the active page container
    if self.ActivePage and self.ActivePage.Container:IsA("ScrollingFrame") then
        self.ActivePage.Container.ScrollingEnabled = true
        self.ActivePage.Container.ScrollBarThickness = 3
        self.ActivePage.Container.ScrollBarImageTransparency = 0.2
        self.ActivePage.Container.CanvasSize = UDim2.new(0, 0, 0, self.ActivePage.Container.UIListLayout.AbsoluteContentSize.Y + 30)
    end
end
...
function Library:ApplyVisualEnhancements()
    if self.GUI then
        -- Enhance shadows
        if self.Main and self.Main:FindFirstChild("Shadow") then
            local shadow = self.Main.Shadow
            Utility:Tween(shadow, {ImageTransparency = 0.4}, 0.3)
...
-- Enhance the UI with these fixes
function Library:EnhanceUI()
    if not self.GUI then return end
    
    -- Fix padding and spacing
    for _, page in pairs(self.Pages) do
        if page.Container:IsA("ScrollingFrame") then
            -- Improve scrolling
            page.Container.ScrollingDirection = Enum.ScrollingDirection.Y
            page.Container.AutomaticCanvasSize = Enum.AutomaticSize.Y
            page.Container.ScrollBarThickness = 3
            page.Container.ScrollBarImageTransparency = 0.2
            
            -- Add smooth scrolling
            local smoothScroll = Utility:Create("NumberValue", {
                Name = "SmoothScroll",
                Value = 0,
            ...
            RunService.RenderStepped:Connect(function()
                if page.Container.ScrollingEnabled and page.Active then
                    local targetScroll = page.Container.CanvasPosition.Y
                    smoothScroll.Value = smoothScroll.Value + (targetScroll - smoothScroll.Value) * 0.2
                    page.Container.CanvasPosition = Vector2.new(0, smoothScroll.Value)
                end
            end)
        end
    end
